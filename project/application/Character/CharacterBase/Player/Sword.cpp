...
void Sword::Update()
{
	...
	// 有効時のみ当たり判定サイズを与える（無効時は 0 で判定なし）。
	// サイズは SetHitHalfSize() で変えられる（スキルは大きめにする）。
...
void Sword::Reattach()
{
	// 手に戻す：親ボーン追従を復帰し、ローカルオフセット・当たり判定を既定へ戻す
	object3d_->SetParentJoint(ownerObj_, ownerJoint_);
	transform.translate = defaultOffsetT_;
	transform.rotate = defaultOffsetR_;
	transform.scale = defaultOffsetS_;
	...
	hitEnabled_ = false;
	hitHalfSize_ = kDefaultHitHalfSize_;
}

void Sword::SetWorldTransform(const Vector3& translate, const Vector3& rotate)
{
	// 切り離し中はこの値がそのままワールド姿勢になる（親が無いため local = world）
